          ...
          <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-20">
            ...
            <div className="lg:col-span-5 space-y-12">
              ...
              <div className="pt-10 border-t border-white/5 space-y-6">
                <p className="text-[10px] font-bold uppercase tracking-widest text-white/40">Broadcasting Channels</p>
            ...
            <div className="lg:col-span-7">
              {!submitted ? (
                <form onSubmit={handleSubmit} className="bg-white/5 border border-white/10 p-10 md:p-16 rounded-[48px] backdrop-blur-xl space-y-8 shadow-2xl relative overflow-hidden group">
                  ...
                  <h3 className="text-3xl font-bold uppercase tracking-tighter">Inquiry Protocol</h3>

                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      <Label className="text-[10px] font-bold uppercase tracking-widest text-white/60 ml-1">Identity / Full Name</Label>
                      <Input required placeholder="Master Chief" className="h-16 rounded-2xl bg-white/5 border-white/10 text-lg font-bold focus:ring-orange-500/50 text-white placeholder:text-white/30" />
                    </div>
                    <div className="space-y-3">
                      <Label className="text-[10px] font-bold uppercase tracking-widest text-white/60 ml-1">Communication Endpoint (Email)</Label>
                      <Input required type="email" placeholder="chief@unsc.mil" className="h-16 rounded-2xl bg-white/5 border-white/10 text-lg font-bold focus:ring-orange-500/50 text-white placeholder:text-white/30" />
                    </div>
                  </div>

                  <div className="space-y-3">
                    <Label className="text-[10px] font-bold uppercase tracking-widest text-white/60 ml-1">Mission Objective / Description</Label>
                    <Textarea required placeholder="Describe your project architecture or branding requirements..." className="min-h-[180px] rounded-2xl bg-white/5 border-white/10 text-lg font-bold p-6 focus:ring-orange-500/50 text-white placeholder:text-white/30" />
                  </div>

                  <Button disabled={loading} className="w-full h-20 rounded-2xl bg-orange-500 hover:bg-orange-600 text-white font-bold uppercase text-sm tracking-widest shadow-2xl shadow-orange-500/20 transition-all active:scale-95">
                    {loading ? "Transmitting..." : "Execute Transmission"} <Send className="ml-3 h-5 w-5" />
                  </Button>

                  <div className="flex items-center justify-center gap-4 pt-4 text-[10px] font-bold uppercase tracking-widest text-white/20">
                ...
                <div className="bg-white/5 border border-white/10 p-20 rounded-[48px] backdrop-blur-xl text-center space-y-10 animate-fade-in shadow-2xl">
                  ...
                  <div className="space-y-4">
                    <h3 className="text-4xl font-bold uppercase tracking-tighter">Transmission Successful</h3>
                  ...
                  <Button onClick={() => setSubmitted(false)} variant="outline" className="rounded-xl font-bold uppercase text-[10px] tracking-widest border-white/10 h-10 px-8">Send Another Signal</Button>
                </div>
              )}
              ...
              <div className="mt-12 flex items-center justify-between p-8 bg-green-500/10 border border-green-500/20 rounded-[32px] group hover:bg-green-500/20 transition-all cursor-pointer shadow-lg shadow-green-500/5">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-green-500/60">Rapid Response Channel</p>
                    <h4 className="text-xl font-bold uppercase tracking-tight text-white group-hover:text-green-400 transition-colors">WhatsApp Direct</h4>
                  </div>
                </div>
...
const ContactItem = ({ icon: Icon, label, value, href }: any) => (
  <a href={href} className="flex items-start gap-6 group/item">
    ...
    <div className="space-y-1">
      <p className="text-[10px] font-bold uppercase tracking-widest text-white/20 group-hover/item:text-orange-500 transition-colors">{label}</p>
      <p className="text-2xl font-bold uppercase tracking-tight text-white">{value}</p>
    </div>
  </a>
);
